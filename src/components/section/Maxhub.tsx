const MAXHUB_SHARE_URL = "https://www.maxhub.com/en/screen_share";
const MAXHUB_CONNECT_URL = "https://connect.maxhub.com/downloads";

export function MaxhubSection({ onClose }: { onClose: () => void }) {
  return (
    <section id="maxhub" className="scroll-mt-24 px-6 pb-20">
      <div className="mx-auto max-w-5xl rounded-2xl border border-neutral-200 bg-white p-8 shadow-sm">
        <div className="flex flex-col gap-6 md:flex-row md:items-start md:justify-between">
          <div>
            <h2 className="text-2xl font-bold tracking-tight text-neutral-900">MaxHub</h2>
            <p className="mt-2 max-w-2xl text-sm text-neutral-600">
              Download the apps for wireless screen sharing and meeting collaboration.
            </p>
          </div>

          <button
            type="button"
            onClick={onClose}
            className="inline-flex w-fit items-center justify-center rounded-full border border-neutral-200 bg-white px-4 py-2 text-sm font-semibold text-neutral-900 hover:bg-neutral-50"
          >
            Back
          </button>
        </div>

        <div className="mt-8 grid gap-4 md:grid-cols-2">
          <a
            href={MAXHUB_SHARE_URL}
            target="_blank"
            rel="noreferrer"
            className="group rounded-xl border border-neutral-200 bg-white p-5 transition hover:border-neutral-300 hover:bg-neutral-50"
          >
            <div className="flex items-start justify-between gap-4">
              <div>
                <div className="text-sm font-semibold text-neutral-900">MAXHUB Screen Share</div>
                <div className="mt-1 text-sm text-neutral-600">Cast your screen to MAXHUB displays.</div>
              </div>
              <span className="text-sm font-semibold text-neutral-900 group-hover:translate-x-0.5 transition">
                Download →
              </span>
            </div>
          </a>

          <a
            href={MAXHUB_CONNECT_URL}
            target="_blank"
            rel="noreferrer"
            className="group rounded-xl border border-neutral-200 bg-white p-5 transition hover:border-neutral-300 hover:bg-neutral-50"
          >
            <div className="flex items-start justify-between gap-4">
              <div>
                <div className="text-sm font-semibold text-neutral-900">MAXHUB Connect</div>
                <div className="mt-1 text-sm text-neutral-600">Share, control, and collaborate in meetings.</div>
              </div>
              <span className="text-sm font-semibold text-neutral-900 group-hover:translate-x-0.5 transition">
                Download →
              </span>
            </div>
          </a>
        </div>
      </div>
    </section>
  );
}
